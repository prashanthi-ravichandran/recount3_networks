...
cancer_consensus_res$Samples_agg <- "Cancerous consensus"
normal_consensus_res$Samples_agg <- "Non-cancerous consensus"
...
context_res <- rbind(blood_consensus_res, cns_consensus_res, adipose_consensus_res, liver_consensus_res, lung_consensus_res, skin_consensus_res)

consensus_res$Samples_agg <- factor(consensus_res$Samples_agg, levels = c("Universal consensus", "Non-cancerous consensus", "Cancerous consensus"))

context_res$Samples_agg <- factor(context_res$Samples_agg, levels = c("Blood", "CNS", "Adipose", "Liver", "Lung", "Skin"))
...
cancer_consensus_summ$Samples_agg <- "Cancerous consensus"
normal_consensus_summ$Samples_agg <- "Non-cancerous consensus"
...
consensus_summ <- rbind(all_consensus_summ, normal_consensus_summ, cancer_consensus_summ)
context_summ <- rbind(blood_consensus_summ, cns_consensus_summ, 
                      adipose_consensus_summ, liver_consensus_summ, 
                      lung_consensus_summ, skin_consensus_summ)
consensus_summ$Samples_agg <- factor(consensus_summ$Samples_agg, levels = c("Universal consensus", "Non-cancerous consensus", "Cancerous consensus"))
context_summ$Samples_agg <- factor(context_summ$Samples_agg, levels = c("Blood", "CNS", "Adipose", "Liver", "Lung", "Skin"))
...
plot_geneSet <- function(gs, df){
   ...
   p <- ggplot(df, aes(x = Decile, y = Enrichment, fill = Samples_agg)) + geom_boxplot() + theme_classic() + geom_hline(yintercept = 1, linewidth = 0.4, linetype = 2) +
        ggtitle(gs) + xlab("Quintile") + ylab("Excess overlap") + scale_fill_brewer(name = "Samples aggregated",palette="Dark2") +
