...
# immune contexts the samples belong to
...
library(stringr)
# Inputs
# 1. data/filter_two_fail_immune.rds
...
filtered_tissue_specific <- readRDS(paste0(homeDir, "data/filter_two_fail_immune.rds"))
...
filtered_tissue_specific$tissue_subtype_inferred <- filtered_tissue_specific$cluster_id
filtered_tissue_specific$tissue_subtype_inferred[filtered_tissue_specific$tissue_subtype_inferred == "1"] <- "myeloid_cells"
filtered_tissue_specific$tissue_subtype_inferred[filtered_tissue_specific$tissue_subtype_inferred == "2"] <- "pbmcs_t_cells"
filtered_tissue_specific$tissue_subtype_inferred[filtered_tissue_specific$tissue_subtype_inferred == "3"] <- "B_cells"

datDir <- paste0(homeDir, "data/")
for(i in unique(filtered_tissue_specific$tissue_subtype_inferred)){
...
for(i in unique(filtered_tissue_specific$tissue_subtype_inferred)){
  print(i)
  df <- filtered_tissue_specific[filtered_tissue_specific$tissue_subtype_inferred == i, ]
  ...
  for(j in unique_GTEx){
    ...
    if(j == "Cells_Ebv-Transformed_Lymphocytes" ){
      j <- "Cells_EBV-transformed_lymphocytes"
    }
    print(j)
