...
rm(list = ls())
.libPaths(c("/data/apps/extern/r-packages/4.2.0", "/home/pravich2/R/x86_64-pc-linux-gnu-library/4.2", 
...
true_tissue_pathways_df <- strsplit(true_tissue_pathways, "_")
true_tissue_pathways_df <- do.call(rbind, true_tissue_pathways_df)

#true_tissue_net <- graph_from_edgelist(true_tissue_pathways)
#true_tissue_net <- as_adjacency_matrix(true_tissue_net)
...
# Compute permuted odds ratio
nPerm <- 100
odds_perm <- list()
for(i in c(1:nPerm)){
  cat(i, "\n")
  perm_tissue_df <- true_tissue_pathways_df
  perm_tissue_df[ ,1] <- sample(perm_tissue_df[ ,1], replace = FALSE)
  perm_tissue_df[ ,2] <- sample(perm_tissue_df[ ,2], replace = FALSE)
  perm_tissue_pathways <- paste(perm_tissue_df[ ,1], perm_tissue_df[ ,2], sep = "_")
  odds <- lapply(edgeList, function(iedge){
    compute_odds(all.edgelist, iedge, perm_tissue_pathways)
  })
  odds_perm[[i]] <- unlist(odds)
}
...
saveRDS(res_odds, paste0(res.dir, tissue, "/", agg_level, "/net_", nStudies, "/res_odds_perm_2.rds"))
