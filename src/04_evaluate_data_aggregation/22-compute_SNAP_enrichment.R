...
edgeList_actual <- edge.list(net)
edgeList_permuted <- edge.permuted.list(net)
nEdges <- lapply(edgeList_actual, function(iedge){
  length(iedge)
})
...
true_positives <- lapply(edgeList_actual, function(iedge){
  length(intersect(iedge, true_tissue_pathways))
})
false_positives <- lapply(edgeList_actual, function(iedge){
  length(iedge) - length(intersect(iedge, true_tissue_pathways))
})
false_negatives <- lapply(edgeList_actual, function(iedge){
  length(true_tissue_pathways) - length(intersect(iedge, true_tissue_pathways))
})
true_negatives <- lapply(edgeList_actual, function(iedge){
  cat(length(iedge), "\n")
  length(intersect(all.edgelist[!all.edgelist %in% true_tissue_pathways],all.edgelist[!all.edgelist %in% iedge]))
})
...
odds_res <- data.frame("Lambda" = as.numeric(lambda),
                       "nEdges" = nEdges,
                       "true_positives" = true_positives, 
                       "false_positives" = false_positives, 
                       "true_negatives" = true_negatives, 
                       "false_negatives" = false_negatives, 
                       "lower_CI" = lower_odds_ratio,
                       "upper_CI" = higher_odds_ratio,
                       "odds_ratio" = odds_ratio,
                       "test_size" = length(true_tissue_pathways))

odds_res$recall <- odds_res$true_positives/(odds_res$true_positives + odds_res$false_negatives)
odds_res$precision <- odds_res$true_positives/(odds_res$true_positives + odds_res$false_positives)
odds_res$F1 <- (2*odds_res$precision*odds_res$recall)/(odds_res$precision + odds_res$recall)

saveRDS(odds_res, paste0(res.dir, tissue, "/", agg_level, "/net_", nStudies, "/oddsRatio.rds"))
