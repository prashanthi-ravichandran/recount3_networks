...
# }
load(file = "/data/abattle4/prashanthi/recount3/data/xcell.data.rda")
...
xCellAnalysis <- function(expr, signatures=NULL, genes=NULL, spill=NULL, rnaseq=TRUE, file.name = NULL, scale=TRUE,
                          alpha = 0.5, save.raw = FALSE, parallel.sz = 1L,
                          cell.types.use = NULL) {
  ...
  scores <- rawEnrichmentAnalysis(expr,signatures,genes,fn, parallel.sz = 1L)
...
rawEnrichmentAnalysis <- function(expr, signatures, genes, file.name = NULL, parallel.sz = 1L) {
  ...
  scores <- GSVA::gsva(expr, signatures, method = "ssgsea",
                       ssgsea.norm = FALSE, parallel.sz = parallel.sz, min.sz = 5)
